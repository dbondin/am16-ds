...
void INSERTSORT(Cats *cats, int size, bool swap(cat & cat1, cat & cat2));
void MERGE(Cats *cats1, Cats *cats2, int s1, int s2, bool swap(cat & cat1, cat & cat2));
void TIMSORT(Cats * cats, int size, bool swap(cat & cat1, cat & cat2));
void FUNCTION(stack<mas>stack, bool swap(cat & cat1, cat & cat2));
...
void TIMSORT(Cats *cats, int size, bool swap(cat & cat1, cat & cat2))
{
	...
	while (i < size) {
		if (size - i >= minrun) {
			...
			INSERTSORT(temp.ptr, temp.size, less);
			...
			INSERTSORT_sort (temp.ptr, temp.size, less);
...
void FUNCTION(stack<massiv> stack, bool swap(cat & cat1, cat & cat2))
{
	if (stack.size () < 3) {
		return;
	}
...
int main ()
{
	...
	while (1) {
		...
		for (unsigned int i = 0; i < strage.length(); i++) {
			if (('0' > strage[i]) || ('9' < strage[i]))
				throw runTIMSORTe_error ("wrong input");
	...
	TIMSORT(cats._data, cats.size (), nameswap);

	PRINTCAT(out, cats._data, cats.size ());
	out.close ();

	out.open ("age.txt", ofstream::binary);
	TIMSORT(cats._data, cats.size (), ageswap);
	PRINTCAT(out, cats._data, cats.size ());
	out.close ();

	out.open ("breed.txt", ofstream::binary);
	TIMSORT(cats._data, cats.size (), breedswap);
	PRINTCAT(out, cats._data, cats.size ());
	out.close ();

	out.open ("food.txt", ofstream::binary);
	TIMSORT(cats._data, cats.size (), foodswap);
	PRINTCAT(out, cats._data, cats.size ());
	out.close ();
